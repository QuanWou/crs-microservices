...
export default function Navbar() {
  ...
  return <nav className="site-nav" aria-label="Điều hướng chính">
    ...
    <div className="nav-links"><Link to="/courses">Môn học</Link>{user?.role === 'ADMIN' && <><Link to="/admin/courses">Quản trị môn học</Link><Link to="/admin/api-keys">API Key</Link></>}{user?.role === 'STUDENT' && <><Link to="/register-course">Đăng ký học phần</Link><Link to="/my-registrations">Đã đăng ký</Link></>}</div>
